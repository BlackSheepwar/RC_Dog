 ...
 * 宏定义与常量
 *============================================================================*/
#define BSP_CAN_MAX_NUM         1           // CAN实例最大数量
...
/**
 * @brief BSP层CAN实例结构体
 * @note 每个实例对应一个CAN外设，通过注册写入静态资源池管理
 */
typedef struct {
    uint8_t  id;                        // CAN实例ID
 ...
 */
typedef struct {
    uint8_t  filter_bank;   // 滤波器序号 (0-13)
...
/**
 * @brief 注册一个CAN实例到资源池
 * @param id   CAN实例ID（用户自定义，不要求等于外设编号）
 ...
 * @param id CAN实例ID
 * @retval 1: 启动成功
 * @retval 0: 启动失败（实例未找到或HAL_CAN_Start失败）
 *
 * @note 启动后自动使能 FIFO0/FIFO1 消息挂起中断，
 *       FreeRTOS 下由信号量驱动任务读取，裸机可配合 BSP_CAN_ReadRxMsg 轮询。
 *       重复调用安全，已启动则直接返回成功。
 */
 ...
 * @retval 1: 配置成功
 * @retval 0: 配置失败（实例未找到/cfg为空/滤波器序号>13/HAL配置失败）
 *
