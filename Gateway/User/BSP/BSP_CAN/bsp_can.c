...
/**
 * @brief 注册一个CAN实例到资源池
 * @param id   CAN实例ID（用户自定义）
 * @param hcan CAN硬件句柄（来自CubeMX生成代码）
 * @retval 1: 注册成功
 * @retval 0: 注册失败
 *
 * @note 注册仅保存句柄，不操作硬件。
 *       启动CAN需额外调用 BSP_CAN_Start。
 */
 ...
 * @retval 0: 启动失败
 *
 * @note 调用HAL_CAN_Start启动CAN外设后，
 *       使能FIFO0和FIFO1的消息挂起中断。
 */
 ...
 * @retval 0: 配置失败
 *
 * @note id1~id4 直接透传到 HAL 的 FilterIdHigh/Low / FilterMaskIdHigh/Low。
 *       mode/scale 决定滤波器工作模式与位宽，用户通过辅助宏组装 id1~id4。
 */
uint8_t BSP_CAN_FilterConfig(uint8_t id, BSP_CAN_FilterConfig_t *cfg)
{
    ...
    if (cfg->filter_bank > 13) return 0;    // 滤波器序号越界
 ...
 *       遥控帧无数据，data 传NULL，len 为期望返回的数据长度。
 */
 ...
 * @note ISR中关闭FIFO0中断防止电平重入风暴，
 *       释放信号量唤醒任务读取，任务读取完毕后重新使能中断。
 */
/**
 * @brief HAL库CAN接收FIFO0消息挂起中断回调
 * @param hcan 触发中断的CAN句柄
